...
def set_web_stuff(client, url):
    client.get(url)
    return client.cookies['csrftoken'], dict(post_client.cookies), {"X-CSRFToken": client.cookies['csrftoken'], "Referer": login_url}

# get config variables
config_data = json.loads(open(args.get('<config>'), 'r').read())
(login_url, get_url, post_url, genome, caller, username, password) = (
    config_data['login_url'], config_data['urlGet'], config_data['urlUp'], config_data['genome'], config_data['caller'],
...
for fn in open(args['<list>'], 'r'):
    ...
    sample1_obj = get_info(bid=bid1, url=get_url)
    sample2_obj = get_info(bid=bid2, url=get_url)
